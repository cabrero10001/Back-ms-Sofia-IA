...
export function errorHandler(err: Error, _req: Request, res: Response, _next: NextFunction): void {
  if (err instanceof ZodError) { const ve = new ValidationError(err.flatten().fieldErrors); res.status(ve.statusCode).json(fail(ve.code, ve.message, ve.details)); return; }
